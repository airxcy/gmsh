...
void mallocConnectivity (struct HXTConnectivity *c, int n, int q)
{
  c-> nNodes = n;
  c-> quantum = q;
  c-> allocated        = (int*)malloc (sizeof(int) * c->nNodes);
  c->degree = (int *)malloc(sizeof(int) * c->nNodes);
  c->nodeConnectivity = (int **)malloc(sizeof(int *) * c->nNodes);
  for (int i=0;i<c->nNodes;i++){
    c-> allocated[i] = c->quantum;
    c-> degree [i] = 0;
    c-> nodeConnectivity [i] = (int*)malloc (sizeof(int) * c-> allocated[i]);
  }
}
...
int addToConnectivity (struct HXTConnectivity *c, int myRow, int myCol){
  ...
  if (c->allocated[myRow] == c->degree[myRow]){
    c->allocated[myRow]*= 2;
    c-> nodeConnectivity [myRow] = (int*)realloc (c-> nodeConnectivity [myRow],sizeof(int) * c-> allocated[myRow]);
  }
...
{
  HXTLinearSystemLU *system = ( HXTLinearSystemLU *)malloc(sizeof(HXTLinearSystemLU));
  *pSystem = system;
  ...
  int *nodeRowStart = (int*)malloc(sizeof(int)*nUsedNodes);
  int *nodeRowEnd = (int *)malloc(sizeof(int) * nUsedNodes);
  ...
  int totalSize = 0;
  system->rowStart = (int *)malloc(sizeof(int) * system->n);
  system->rowEnd = (int *)malloc(sizeof(int) * system->n);
  system->rowLuEnd = (int *)malloc(sizeof(int) * system->n);
  for (int i = 0; i < nUsedNodes; ++i) {
  ...
  system->M = (double*)malloc(sizeof(double)*totalSize); // FIXME Gmsh
  system->rows = (double **)malloc(sizeof(double *) * system->n);
  ...
  system->x = (double *)malloc(sizeof(double) * system->n); // FIXME Gmsh
