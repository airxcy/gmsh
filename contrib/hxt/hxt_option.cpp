	  ...
	  (opt->argRequirement==ARG_OPTIONAL && arg!=NULL)) {
		...
		if(errno == ERANGE){
			return HXT_ERROR_MSG(HXT_STATUS_RANGE_ERROR,
			       ...
			       arg, optName, getArgTypeName((HXT_ARG_TYPE)opt->argType));
		}
		else if (arg == endptr)
		{
			...
			case ARG_INT32:
			{
				...
				if(i32!=integer)
					return HXT_ERROR_MSG(HXT_STATUS_RANGE_ERROR,
					       ...
					       arg, optName, getArgTypeName((HXT_ARG_TYPE)opt->argType), integer, integer);
			}
			break;
			case ARG_UINT32:
			{
			...
			case ARG_FLOAT:
			{
				...
				if(f32!=real)
					return HXT_ERROR_MSG(HXT_STATUS_RANGE_ERROR,
					       ...
					       arg, optName, getArgTypeName((HXT_ARG_TYPE)opt->argType), real, real);
			}
			break;
			default:
...
HXTStatus hxtOptCalledWithoutArg(HXTOpt* opt){
	if(opt->argType>-4) // integer type
        return (HXTStatus)(opt->integer == INT64_MIN);
	else if(opt->argType>-8)
        return (HXTStatus)(opt->real == NAN);
	else
        return (HXTStatus)(opt->string[0] == '\a');
}
